"""Build runtime-ready identity-preserving pose sheets for female_01.
...
SOURCE = ROOT / "tmp" / "female_01_pose_sheet_imagegen_v1.png"
MOTION_DIR = ROOT / "assets" / "characters" / "motion_sheets"
REVIEW_DIR = ROOT / "output" / "asset_reviews" / "female_01_pose_cycles_v2"
...
ACTION_MAP = {
    "crawl": (1, 2, 3, 4, 3, 2, 4, 1),
    "climb": (1, 2, 5, 5, 5, 2, 1, 1),
    "sit": (0, 1, 2, 6, 6, 6, 1, 0),
...
def _contact_sheet(action: str, sheet: Image.Image) -> Image.Image:
    ...
    draw.text((16, 14), f"female_01 / {action} / v2", fill=(35, 45, 58), font=font)
...
def main() -> None:
    ...
    for action, mapping in ACTION_MAP.items():
        ...
        filename = f"female_01_{action}_cycle_v2.png"
        sheet.save(MOTION_DIR / filename, format="PNG", optimize=True)
        _contact_sheet(action, sheet).save(REVIEW_DIR / f"{filename[:-4]}_contact.png", format="PNG", optimize=True)
        records.append({
            "action": action,
    ...
    (MOTION_DIR / "female_01_pose_cycles_v2_manifest.json").write_text(
        json.dumps({
            "version": "v2",
            "character_key": "female_01",
            "source": "tmp/female_01_pose_sheet_imagegen_v1.png",
            "transparent_runtime_contract": {"columns": COLUMNS, "rows": ROWS, "cell_size": [CELL_WIDTH, CELL_HEIGHT]},
            "assets": records,
        }, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(f"Built {len(records)} pose cycles for female_01 in {MOTION_DIR}")
